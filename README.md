# Infinite Glade

[![Release](https://img.shields.io/github/v/release/romrem30-eng/InfiniteGlade?style=for-the-badge&color=brightgreen)](https://github.com/romrem30-eng/InfiniteGlade/releases)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg?style=for-the-badge)](https://opensource.org/licenses/MIT)
[![Target: Tiny Glade](https://img.shields.io/badge/Game-Tiny%20Glade%20v1.16.0-blue?style=for-the-badge)](https://store.steampowered.com/app/2198150/Tiny_Glade/)
[![Status: Proof of Concept](https://img.shields.io/badge/Status-Proof%20of%20Concept-orange?style=for-the-badge)](https://github.com/romrem30-eng/InfiniteGlade/issues)

Infinite Glade is a native gameplay and camera overhaul modification for Tiny Glade, removing boundary constraints and expanding the building canvas into an endless flat horizon with 150-meter stratosphere camera zoom.

> [!NOTE]
> **Project Status: Proof of Concept (PoC / Tech Preview)**  
> This project demonstrates that Tiny Glade's internal boundary and camera constraints can be unlocked at runtime without sacrificing engine stability or rendering performance. Active development and community feedback are welcome.

For an exhaustive technical breakdown of the reverse engineering process, see [Technical Deep Dive](docs/TECHNICAL_DEEP_DIVE.md).

---

## Key Features

- **Infinite Build Area**: Construct castles, houses, walls, towers, roofs, stairs, and terrain features across the entire landscape without border collisions.
- **Free Camera and 3.7x Zoom**: Unlocked camera panning past the clearing perimeter and extended maximum zoom distance from 40m to 150m.
- **Flat Horizon Canvas**: The default 27-meter background terrain hills are flattened to Y = 0.0, creating an unobstructed building plane.
- **Zero Clutter / Clean Skirt**: Photomode border stones, Z-fighting mesh artifacts, 2D billboards, and 1,500+ distant background bushes are submerged underground.
- **Anti-Tamper Bypass**: Integrated runtime integrity check bypass for stable execution at 60+ FPS.

---

## Verified Test Rig and Performance Benchmark

The modification was profiled on mainstream mobile gaming hardware, confirming stable framerates under heavy build loads:

| Component | Specification |
| :--- | :--- |
| **CPU** | AMD Ryzen 5 4600H (6 Cores / 12 Threads @ up to 4.0 GHz) |
| **GPU** | NVIDIA GeForce GTX 1650 Ti (4 GB VRAM) |
| **RAM** | 16 GB DDR4 |
| **OS** | Windows 11 / 10 (x64) |
| **Target Build** | Steam v1.16.0-pre4 |
| **Framerate** | Solid 60 FPS during active building, terrain painting, and 150m stratosphere zoom |

---

## Installation

1. Download `InfiniteGlade-v1.1.0.zip` from [Releases](https://github.com/romrem30-eng/InfiniteGlade/releases).
2. Extract the archive contents into your Tiny Glade root directory:
   - Default Steam path: `C:\Program Files (x86)\Steam\steamapps\common\Tiny Glade\`
3. Run `install.bat` (or copy `GladeLoader.exe`, `glade_loader.dll`, and the `assets\` directory manually).
4. Launch the game using `GladeLoader.exe` (or your standard shortcut).

---

## Save Game Safety and Backup Advice

> [!IMPORTANT]
> **Always back up your saves before creating massive out-of-bounds projects.**  
> Tiny Glade serializes all objects placed outside the glade into standard world saves. However, if you launch the game in vanilla mode (without the loader), vanilla boundary checks will prevent you from editing or extending structures located beyond the 40m perimeter.

- **Save Folder Location:**  
  `%USERPROFILE%\Saved Games\Tiny Glade\Steam\<YourSteamID>\saves\`
- To back up worlds, create a copy of the `saves` directory.

---

## Known Edge Cases and Technical Notes

1. **Static RVAs vs. Official Game Updates:**  
   The current release uses hardcoded Relative Virtual Addresses (RVAs) matched against Steam build `v1.16.0-pre4`.  
   If an official game patch alters function offsets and the loader fails to hook, please open an [Issue](https://github.com/romrem30-eng/InfiniteGlade/issues). Dynamic array-of-bytes (AOB) signature scanning is planned for subsequent versions.
2. **Horizon Edge Visibility at 150m:**  
   Because the background 27m obstructive hills have been flattened to Y = 0.0, pulling the camera out to the maximum 150m stratosphere distance will expose the circular edge of the procedural ground mesh.
3. **VRAM Scaling with Large Structures:**  
   Tiny Glade utilizes a real-time procedural voxel meshing pipeline (Marching Cubes / SDF). When constructing monumental cities with thousands of structural elements, monitor GPU VRAM usage on 4GB cards.

---

## Technical Overview

Tiny Glade is written in Rust on the Bevy ECS engine with a custom Vulkan renderer (Rhapsody). The modification operates across two layers:

1. **Native Runtime Patching (`glade_loader.dll`)**:
   - Injected into `tiny-glade.exe` at startup via remote thread injection.
   - Bypasses the binary integrity check loop (`RVA +0x176F5F`).
   - Patches 4 border constraint methods to return `true` (`mov al, 1; ret`):
     - `is_within_glade_shape` (`RVA +0x9D4A10`)
     - `GladeBorder::is_pos_inside` (`RVA +0x91C400`)
     - `GladeBorder::is_shape_inside` (`RVA +0x91C420`)
     - `GladeBorder::is_curve2_inside` (`RVA +0x91C710`)
   - Unlocks camera pan delta and perimeter clamps; increases max zoom to 150.0m (`RVA +0x2EB6074`).
   - Suppresses border debris by intercepting `display_glade_border` (`RVA +0x2069E41`).

2. **Horizon Mesh Engineering (`assets/meshes/*.json`)**:
   - `terrain.json`: 1,078 skirt vertices normalized to Y = 0.0 with up-normals `[0, 1, 0]`.
   - `terrain_rocks.json`, `billboard_plants*.json`, `far_distance_tree.json`: Vertices translated to Y = -500.0. Buffer sizes and vertex counts are preserved to prevent Vulkan storage buffer allocations from failing (`assertion failed: size > 0`).

---

## Building from Source

Requirements:
- Rust (stable toolchain with 2024 edition support)
- Visual Studio Build Tools / MSVC x64

```bash
cd glade_loader
cargo build --release
```

Output binaries will be located at `target/release/GladeLoader.exe` and `target/release/glade_loader.dll`.

---

## Community and Feedback

- Report bugs or submit compatibility requests: [GitHub Issues](https://github.com/romrem30-eng/InfiniteGlade/issues)
- Share screenshots and discuss architecture: [GitHub Discussions](https://github.com/romrem30-eng/InfiniteGlade/discussions)

---

## License

This project is licensed under the [MIT License](LICENSE).
